...
*/
import java.util.Random; // for random spawn counts
...
public class Level { // represents one level in the game
	...
	private Random rng = new Random(); // random object for spawn ranges
	...
	public int getRandomThrowableCount() { // pick a number between min and max
	    int min = getMinThrowables(); 
	    int max = getMaxThrowables();

	    return rng.nextInt(max - min + 1) + min; // easy random throwable formula incase I was to edit ranges
	}
