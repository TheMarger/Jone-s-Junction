...
This file and class are used to house the logic of the Rectangular Guard's FOV (Field of View). It contains FOV calculations and states when this logic applies.
...
public class RectangularGuard extends gaurd { // Declares the RectangularGuard class, which extends the base guard class

    public RectangularGuard(gamePanel gp) { // Constructor that takes the gamePanel so the guard can access game data
        super(gp); // Calls the parent guard constructor to initialize shared guard properties
        name = "Rectangular Guard"; // Sets this guard's display name to identify its type
    }

    @Override
    public Rectangle getFOV() { // Overrides the base guard FOV method to define a rectangular field of view
        ...
        switch (direction) { // Checks which direction the guard is currently facing
            case "up": // If the guard is facing upward
                fov.setBounds(worldX - ts, worldY - 2 * ts, 3 * ts, 2 * ts); // Creates a wide, short FOV above the guard
                break; // Ends this case

            case "down": // If the guard is facing downward
                fov.setBounds(worldX - ts, worldY + ts, 3 * ts, 2 * ts); // Creates a wide, short FOV below the guard
                break; // Ends this case

            case "left": // If the guard is facing left
                fov.setBounds(worldX - 3 * ts, worldY - ts, 3 * ts, 2 * ts); // Creates a long, narrow FOV to the left
                break; // Ends this case

            case "right": // If the guard is facing right
                fov.setBounds(worldX + ts, worldY - ts, 3 * ts, 2 * ts); // Creates a long, narrow FOV to the right
                break; // Ends this case
        }
